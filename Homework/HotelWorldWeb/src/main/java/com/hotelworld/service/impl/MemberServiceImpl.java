...
public class MemberServiceImpl implements MemberService {
    ...
    public WebMessage register(Member member) {
        ...
        if (memberDB != null) {
            return new WebMessage("用户名已经存在！", WebMessageState.WARNING);
        } else {
            dao1.saveMember(member);
            MemberLog log = new MemberLog(IdUtil.getMemberLogId(), MemberOperateType.REGISTER,
                    0, DateUtil.getToday(),1);
            dao2.saveMemberLog(log);
            return new WebMessage("注册成功！", WebMessageState.NORMAL);
        }
    }
    ...
    public WebMessage join(Member member) {
        Member memberDB = dao1.findMemberById(member.getId());
        if (memberDB == null) {
            return new WebMessage("用户不存在！", WebMessageState.WARNING);
        } else {
            member.setState(MemberState.INACTIVE);
            dao1.updateMember(member);
            MemberLog log = new MemberLog(IdUtil.getMemberLogId(), MemberOperateType.JOIN,
                    0, DateUtil.getToday(),1);
            return new WebMessage("成功开通会员。会员状态：未激活", WebMessageState.NORMAL);
        }
    }
    ...
    public WebMessage stop(Member member) {
        ...
        if (memberDB == null) {
            ...
            dao1.updateMember(member);
            MemberLog log = new MemberLog(IdUtil.getMemberLogId(), MemberOperateType.STOP,
                    0, DateUtil.getToday(),1);
            dao2.saveMemberLog(log);
            return new WebMessage("成功停止会员。", WebMessageState.NORMAL);
        }
    }
    ...
    public WebMessage recharge(Member member, int money) {
        ...
        if (memberDB == null) {
            ...
            return new WebMessage("用户还未激活！",WebMessageState.WARNING);
        } else{
            ...
            String message = "";
            WebMessageState mState = WebMessageState.NORMAL;
            if (state == MemberState.ACTIVE) {
                ...
                if (balance < 1000) {
                    ...
                    MemberLog log = new MemberLog(IdUtil.getMemberLogId(), MemberOperateType.RECHARGE,
                    ...
                    MemberLog log1 = new MemberLog(IdUtil.getMemberLogId(), MemberOperateType.RECHARGE,
                            money, DateUtil.getToday(),1);
                    MemberLog log2 = new MemberLog(IdUtil.getMemberLogId(), MemberOperateType.ACTIVATE,
                            0, DateUtil.getToday(),1);
